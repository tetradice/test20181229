...
import { WiredActions, ParamType, ActionImplements, 
...
// View
const view = (state: MyState, acts: MyActions) => {
  let currentState = actions.down(1);
  acts.propFunc({a: '1', b: '2'});
  acts.tags.getLabel();

  return (
    <div>
      <h1>{state.count}</h1>
      <button onclick={() => acts.down(1)}>-</button>
      <button onclick={() => acts.up(1)}>+</button>
      <button onclick={() => acts.reset()}>Reset</button>
    </div>
  )
}
