...
// 指定したメガミのカードIDリストを取得
export function getMegamiCardIds(megami: Megami, baseType: null | CardDataItem['baseType'], includeExtra = false){
    let ret = [];

    // 全カード情報を取得
    let allCardDataItem: CardDataItem[] = [];
    for(let key in CARD_DATA){
        allCardDataItem.push(CARD_DATA[key]);
    }

    // 全カードを探索し、指定されたメガミと対応するカードを、カードIDリストへ追加する
    for(let key in CARD_DATA){
        let data = CARD_DATA[key];
