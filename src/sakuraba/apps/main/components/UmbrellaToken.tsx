...
export const UmbrellaToken = (p: {side: PlayerSide, umbrellaState: UmbrellaState, left: number, top: number}) => (state: state.State, actions: ActionsType) => {
    ...
    return <img class={(p.side === state.side ? 'clickable' : '')} src={`//inazumaapps.info/furuyoni_simulator/deliv/furuyoni_commons/furuyoni_na/cards/umbrella_${p.umbrellaState === 'closed' ? 'a' : 'b'}_cut.png`} style={styles} onclick={onclick} />;
}