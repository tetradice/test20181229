...
export const MachineButtons = (p: {side: PlayerSide, left: number, top: number}) => (state: state.State, actions: ActionsType) => {
    ...
    return (
        <div style={styles}>
            <button class={`mini ui basic button${machineTokens.length === 0 || distanceTokens.length <= 0 ? ' disabled' : ''}`} onclick={charge}>騎動前進</button>
            <button class={`mini ui basic button${machineTokens.length === 0 || distanceTokens.length >= 10 ? ' disabled' : ''}`} onclick={charge}>騎動後退</button>
        </div>
    );
}