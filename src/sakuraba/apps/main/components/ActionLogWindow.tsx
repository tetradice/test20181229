...
export const ActionLogWindow = (p: {shown: boolean, logs: state.ActionLogRecord[]}) => (state: state.State, actions: ActionsType) => {
    if(p.shown){
        ...
        p.logs.forEach((log) => {
            ...
            logElements.push(
                <div>
                {name}: <span style={bodyStyle}>{utils.translateLog(log.body, state.detectedLanguage, state.setting.language)}</span> <span style={{fontSize: 'smaller', color: 'silver'}}>({timeStr})</span>
                </div>
            )
        });
